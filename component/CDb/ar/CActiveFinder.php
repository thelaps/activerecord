...
class CActiveFinder extends CComponent
{
	...
	private function buildJoinTree($parent,$with,$options=null)
	{
		if($parent instanceof CStatElement)
			throw new \Exception('The STAT relation "'.$parent->relation->name.'" cannot have child relations.');
		...
		if(is_string($with))
		{
			...
			if(($relation=$parent->model->getActiveRelation($with))===null)
				throw new \Exception('Relation "'.get_class($parent->model).'" is not defined in active record class "'.$with.'".');
	...
	private function applyLazyCondition($query,$record)
	{
		...
		if($this->relation instanceof CManyManyRelation)
		{
			...
			if(($joinTable=$schema->getTable($joinTableName))===null)
				throw new \Exception('The relation "'.$this->relation->name.'" in active record 
				class "'.get_class($parent->model).'" is not specified correctly: the join table "'.$joinTableName.'" 
			...
			if($parentCondition!==array() && $childCondition!==array())
			{
				...
				throw new \Exception('The relation "'.$this->relation->name.'" in active record 
				class "'.get_class($parent->model).'" is specified with an incomplete foreign key. The foreign key 
				must consist of columns referencing both joining tables.');
		}
		else
		{
			$element=$this;
		...
		else
		{
			...
			foreach($select as $name)
			{
				...
				elseif(preg_match('/^(.*?)\s+AS\s+(\w+)$/im',$name,$matches)) // if the column is already aliased
				{
					...
					throw new \Exception('Active record "'.get_class($this->model).'" is trying to select an invalid 
					column "'.$name.'". Note, the column must exist in the table or be an expression with alias.');
			}
	...
	private function joinOneMany($fke,$fks,$pke,$parent)
	{
		...
		foreach($fks as $i=>$fk)
		{
			...
			if(!isset($fke->_table->columns[$fk]))
				throw new \Exception('The relation "'.$this->relation->name.'" in active record 
				class "'.get_class($parent->model).'" is specified with an invalid foreign key "'.$fk.'". There is no 
				such column in the table "'.$fke->_table->name.'".');
	...
	private function joinManyMany($joinTable,$fks,$parent)
	{
		...
		foreach($fks as $i=>$fk)
		{
			if(!isset($joinTable->columns[$fk]))
				throw new \Exception('The relation "'.$this->relation->name.'" in active record 
				class "'.get_class($parent->model).'" is specified with an invalid foreign key "'.$fk.'". 
				There is no such column in the table "'.$joinTable->name.'".');
		...
		if($parentCondition!==array() && $childCondition!==array())
		{
			...
			throw new \Exception('The relation "'.$this->relation->name.'" in active record 
			class "'.get_class($parent->model).'" is specified with an incomplete foreign key. The foreign key must 
			consist of columns referencing both joining tables.');
	}
}
...
class CStatElement
{
	...
	private function queryOneMany()
	{
		...
		if(count($fks)!==count($pkTable->primaryKey))
			throw new \Exception('The relation "'.$relation->name.'" in active record 
			class "'.get_class($parent->model).'" is specified with an invalid foreign key. The columns in the key must 
			match the primary keys of the table "'.$pkTable->name.'".');
		...
		foreach($fks as $i=>$fk)
		{
			if(!isset($table->columns[$fk]))
				throw new \Exception('The relation "'.$relation->name.'" in active record 
				class "'.get_class($parent->model).'" is specified with an invalid foreign key "'.$fk.'". There is no 
				such column in the table "'.$table->name.'".');
			...
			if(isset($table->foreignKeys[$fk]))
			{
				...
				if($schema->compareTableNames($pkTable->rawName,$tableName))
					...
					throw new \Exception('The relation "'.$relation->name.'" in active record 
					class "'.get_class($parent->model).'" is specified with a foreign key "'.$fk.'" that does not point 
					to the parent table "'.$pkTable->name.'".');
			}
			else  // FK constraints undefined
			{
	...
	private function queryManyMany($joinTableName,$keys)
	{
		...
		if(($joinTable=$builder->getSchema()->getTable($joinTableName))===null)
			throw new \Exception('The relation "'.$relation->name.'" in active record 
			class "'.get_class($this->_parent->model).'" is not specified correctly: the join table "'.$joinTableName.'" 
			given in the foreign key cannot be found in the database.');

		$fks=preg_split('/\s*,\s*/',$keys,-1,PREG_SPLIT_NO_EMPTY);
		if(count($fks)!==count($table->primaryKey)+count($pkTable->primaryKey))
			throw new \Exception('The relation "'.$relation->name.'" in active record 
			class "'.get_class($this->_parent->model).'" is specified with an incomplete foreign key. The foreign key 
			must consist of columns referencing both joining tables.');
		...
		foreach($fks as $i=>$fk)
		{
			if(!isset($joinTable->columns[$fk]))
				throw new \Exception('The relation "'.$relation->name.'" in active record 
				class "'.get_class($this->_parent->model).'" is specified with an invalid foreign key "'.$fk.'". There 
				is no such column in the table "'.$joinTable->name.'".');
