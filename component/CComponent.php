...
use component\CDb\ar\CModelEvent;
        ...
        if(isset($this->_e[$name]))
        {
            foreach($this->_e[$name] as $handler)
            {
                ...
                elseif(is_callable($handler,true))
                {
                    ...
                    throw new \Exception('Event "'.get_class($this).'.'.$name.'" is attached with an invalid handler "'.gettype($handler).'".');
                // stop further handling if param.handled is set true
                if(($event instanceof CModelEvent) && $event->handled)
                    return;
            }
        }
